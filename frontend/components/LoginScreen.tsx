...
import { Plane, Mail, Lock, Camera, Coffee, Mountain, Landmark, UtensilsCrossed, Waves, Moon, Leaf, MapPin, ArrowLeft, Check } from 'lucide-react';
...
const travelPreferences: TravelPreference[] = [
  { id: 'photography', label: 'Chụp hình', icon: <Camera className="w-4 h-4" />, color: 'bg-yellow-100 text-yellow-700 border-yellow-200' },
  { id: 'coffee', label: 'Cà phê', icon: <Coffee className="w-4 h-4" />, color: 'bg-amber-100 text-amber-700 border-amber-200' },
  { id: 'drink', label: 'Đồ uống', icon: <Coffee className="w-4 h-4" />, color: 'bg-amber-100 text-amber-700 border-amber-200' },
  { id: 'trekking', label: 'Trekking', icon: <Mountain className="w-4 h-4" />, color: 'bg-green-100 text-green-700 border-green-200' },
  { id: 'museum', label: 'Bảo tàng / Nghệ thuật', icon: <Landmark className="w-4 h-4" />, color: 'bg-purple-100 text-purple-700 border-purple-200' },
  { id: 'food', label: 'Ẩm thực địa phương', icon: <UtensilsCrossed className="w-4 h-4" />, color: 'bg-orange-100 text-orange-700 border-orange-200' },
  { id: 'beach', label: 'Biển / nghỉ dưỡng', icon: <Waves className="w-4 h-4" />, color: 'bg-cyan-100 text-cyan-700 border-cyan-200' },
  { id: 'nightlife', label: 'Nightlife', icon: <Moon className="w-4 h-4" />, color: 'bg-indigo-100 text-indigo-700 border-indigo-200' },
  { id: 'relaxed', label: 'Chill style', icon: <Leaf className="w-4 h-4" />, color: 'bg-teal-100 text-teal-700 border-teal-200' },
  { id: 'culture', label: 'Khám phá văn hóa', icon: <MapPin className="w-4 h-4" />, color: 'bg-pink-100 text-pink-700 border-pink-200' },
];
...
export function LoginScreen({ onLogin }: LoginScreenProps) {
  ...
  const renderStep1 = () => (
    <div className="bg-white rounded-2xl shadow-lg p-8">
      <h2 className="mb-2">{isRegister ? 'Tạo tài khoản mới' : 'Chào mừng trở lại'}</h2>
      <p className="text-muted-foreground mb-6">
        {isRegister ? 'Đăng ký để bắt đầu lên kế hoạch' : 'Đăng nhập để tiếp tục'}
      </p>
      ...
      <div className="mt-6 text-center">
        ...
        >
          {isRegister ? (
            <>Đã có tài khoản? <span className="text-[#0066FF]">Đăng nhập ngay</span></>
          ) : (
            <>Chưa có tài khoản? <span className="text-[#0066FF]">Đăng ký ngay</span></>
          )}
        </button>
      </div>
    </div>
    ...
    <div className="w-full">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl shadow-lg p-8">
            <h2 className="mb-2">Hoàn thiện hồ sơ của bạn để TravelGPT hiểu bạn hơn!</h2>
            <p className="text-muted-foreground mb-6">
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-[#F8FAFC]">
        ...
        <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-[#0066FF] to-[#00C29A] p-12 items-center justify-center relative overflow-hidden">
          ...
          <div className="relative z-10 text-white text-center">
            ...
            <p className="text-white/90 max-w-md mx-auto">
              Your smart AI travel companion for planning unforgettable adventures. 
      ...
      {/* Right side - Form */}
      <div className={`flex-1 flex items-center justify-center p-6 lg:p-12 ${isRegister && registrationStep === 2 ? 'lg:col-span-2' : ''}`}>
        <div className={`w-full ${isRegister && registrationStep === 2 ? 'max-w-6xl' : 'max-w-md'}`}>
          {/* Mobile Logo */}
