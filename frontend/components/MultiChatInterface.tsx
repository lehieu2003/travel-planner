...
import { Send, Menu, Calendar } from 'lucide-react';
...
export function MultiChatInterface({
  ...
  const getItineraryData = () => {
    ...
    const messageWithItinerary = [...messages]
      ...
      .find(
        (m) =>
          m.itineraryData &&
          m.itineraryData.days &&
          m.itineraryData.days.length > 0,
      );
      ...
      setMessages([
        {
          ...
          role: 'assistant',
          content:
            'Hãy nói cho mình biết bạn muốn đi đâu và ngân sách thế nào nhé ✈️✨',
  ...
  const handleRenameConversation = async (id: string, newTitle: string) => {
    try {
      const response = await fetch(
        API_ENDPOINTS.CONVERSATIONS.UPDATE_TITLE(id),
        {
          method: 'PATCH',
          headers: getAuthHeaders(),
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (data.ok && data.is_list) {
        ...
        if (data.conversation_id) {
          ...
          setMessages((prev) => {
            ...
            const existingAssistantIndex = filtered.findIndex(
              (msg, idx) =>
                msg.role === 'assistant' && idx === filtered.length - 1,
            );
        ...
        data.ok &&
        (data.requires_clarification || data.requires_confirmation)
      ) {
  ...
  return (
    <div className='h-screen flex bg-background'>
      ...
      <Button
        variant='ghost'
        size='icon'
        className='md:hidden fixed top-4 left-4 z-30 bg-white dark:bg-card shadow-md rounded-xl'
      ...
      {hasItinerary && (
        <Button
          variant='ghost'
          size='icon'
          className='lg:hidden fixed top-4 right-4 z-30 bg-white dark:bg-card shadow-md rounded-xl'
      ...
      <div className='flex-1 flex flex-col bg-background overflow-hidden min-h-0'>
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-2'>
                    {quickPrompts.map((prompt, idx) => (
                      <motion.button
                        ...
                        className='px-4 py-3 bg-white dark:bg-card hover:bg-primary/5 dark:hover:bg-primary/10 border border-border rounded-xl transition-all duration-200 hover:border-primary/30 hover:shadow-sm text-left'
                      >
                        {prompt}
                      </motion.button>
                    ))}
        ...
        {/* Input Area */}
        <div className='border-t border-border bg-white dark:bg-card px-4 md:px-6 py-4'>
          <div className='max-w-4xl mx-auto'>
            <div className='flex gap-2'>
              <div className='flex-1 relative'>
                <textarea
                  ...
                  rows={1}
                  className='w-full px-4 py-3 pr-12 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary resize-none bg-white dark:bg-muted text-foreground transition-all duration-200'
        ...
        {showItinerary && hasItinerary && (
          <>
            ...
            <motion.div
              ...
              className='hidden lg:block w-[400px] xl:w-[450px] border-l border-border bg-white dark:bg-card relative z-10'
            >
              <ItineraryPanel
                key={itineraryKey}
            ...
            >
              <motion.div
                ...
                exit={{ y: '100%' }}
                transition={{ type: 'spring', damping: 30, stiffness: 300 }}
                className='absolute bottom-0 left-0 right-0 top-0 bg-white dark:bg-card rounded-t-3xl overflow-hidden'
