...
export function ChatMessage({ message, onSaveItinerary }: ChatMessageProps) {
  ...
  const hasHTML = !isUser && (message.content.includes('<b>') || message.content.includes('</b>') || message.content.includes('<strong>') || message.content.includes('</strong>'));
      ...
      <div className={`max-w-[75%] ${isUser ? 'order-first' : ''}`}>
        ...
        <div
          className={`rounded-2xl px-5 py-3 shadow-sm transition-all duration-200 ${
            isUser
              ? 'bg-[#EDF2FF] text-foreground ml-auto'
