...
import { Edit, Mail, User, Coffee, Waves, Calendar, Camera, Mountain, Landmark, UtensilsCrossed, Moon, Leaf, MapPin, X, Check, Activity, Zap, DollarSign } from 'lucide-react';
...
const travelPreferences: TravelPreference[] = [
  { id: 'photography', label: 'Chụp hình', icon: Camera, color: 'bg-yellow-100 text-yellow-700 border-yellow-200' },
  { id: 'coffee', label: 'Cà phê', icon: Coffee, color: 'bg-amber-100 text-amber-700 border-amber-200' },
  { id: 'drink', label: 'Đồ uống', icon: Coffee, color: 'bg-amber-100 text-amber-700 border-amber-200' },
  { id: 'trekking', label: 'Trekking', icon: Mountain, color: 'bg-green-100 text-green-700 border-green-200' },
  { id: 'museum', label: 'Bảo tàng / Nghệ thuật', icon: Landmark, color: 'bg-purple-100 text-purple-700 border-purple-200' },
  { id: 'food', label: 'Ẩm thực địa phương', icon: UtensilsCrossed, color: 'bg-orange-100 text-orange-700 border-orange-200' },
  { id: 'beach', label: 'Biển / nghỉ dưỡng', icon: Waves, color: 'bg-cyan-100 text-cyan-700 border-cyan-200' },
  { id: 'nightlife', label: 'Nightlife', icon: Moon, color: 'bg-indigo-100 text-indigo-700 border-indigo-200' },
  { id: 'relaxed', label: 'Chill style', icon: Leaf, color: 'bg-teal-100 text-teal-700 border-teal-200' },
  { id: 'culture', label: 'Khám phá văn hóa', icon: MapPin, color: 'bg-pink-100 text-pink-700 border-pink-200' },
];
...
export function Profile() {
  ...
  const selectedPrefsData = travelPreferences.filter(p => selectedPreferences.includes(p.id));
  ...
  const stats = profileData?.stats || { tripsPlanned: 0, placesVisited: 0, savedItineraries: 0 };
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          ...
          <div className="grid grid-cols-3 gap-4 pt-6 border-t border-border">
            ...
            <div className="text-center">
              <div className="text-[#0066FF] mb-1">{stats.savedItineraries}</div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-muted-foreground">
                {isEditing ? 'Chọn những gì bạn yêu thích' : 'Sở thích đã lưu của bạn'}
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          ...
          <div className="space-y-3">
            {[
              { query: 'Đi Đà Lạt 3 ngày ngân sách 5 triệu', date: '2 ngày trước' },
              { query: 'Hà Nội cuối tuần thích cà phê', date: '5 ngày trước' },
              { query: 'Phú Quốc 2 ngày 1 đêm chill', date: '1 tuần trước' },
            ].map((item, index) => (
              <div
                key={index}
