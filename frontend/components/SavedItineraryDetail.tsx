...
import { 
  ArrowLeft, Calendar, DollarSign, MapPin, Clock, Star, Navigation, 
  Edit, Share2, FileDown, Trash2, Coffee, Utensils, Mountain, 
  Landmark, Camera, Heart, Users, Bus, ChevronRight, Copy, MessageSquare,
  Hotel, Zap, Tag
} from 'lucide-react';
...
interface SavedItinerary {
  ...
  payload: {
    ...
    categorized_places?: {
      places?: Array<{ name: string; description?: string; address?: string; rating?: number }>;
      food?: Array<{ name: string; description?: string; address?: string; rating?: number }>;
      coffee?: Array<{ name: string; description?: string; address?: string; rating?: number }>;
      drink?: Array<{ name: string; description?: string; address?: string; rating?: number }>;
    };
  };
  created_at: string;
}
...
const extractCategorizedPlaces = (days?: Array<{ activities: Array<any> }>) => {
  const places: Array<{ name: string; description?: string; address?: string; rating?: number }> = [];
  const food: Array<{ name: string; description?: string; address?: string; rating?: number }> = [];
  const coffee: Array<{ name: string; description?: string; address?: string; rating?: number }> = [];
  const drink: Array<{ name: string; description?: string; address?: string; rating?: number }> = [];
  ...
  days.forEach(day => {
    day.activities?.forEach((activity: any) => {
      ...
      if (category === 'food' || category === 'restaurant') {
        food.push(placeInfo);
      } else if (category === 'drink' || category === 'coffee' || category === 'cafe') {
...
const parseTimeRange = (
...
): { startTime: string; endTime: string; calculatedDurationMinutes: number } | null => {
  ...
  if (timeStr) {
    ...
    if (rangeMatch) {
      ...
      const calculatedDuration = calculateDurationFromTimeRange(startTime, endTime);
...
export function SavedItineraryDetail({ itineraryId, onBack }: SavedItineraryDetailProps) {
  ...
  const loadItinerary = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await fetch(API_ENDPOINTS.ITINERARIES.GET(parseInt(itineraryId)), {
      ...
      <div className="min-h-screen bg-[#F8FAFC]">
        ...
        <div className="max-w-7xl mx-auto p-4 sm:p-6">
          <div className="bg-white rounded-2xl shadow-sm p-8 text-center">
            <p className="text-lg text-muted-foreground">{error || 'Không tìm thấy lịch trình.'}</p>
  ...
  const categorizedPlaces = payload.categorized_places || extractCategorizedPlaces(payload.days);
  
  // Calculate total days and nights from duration string or days array
  const durationMatch = payload.duration?.match(/(\d+)\s*ngày/);
  const totalDays = durationMatch ? parseInt(durationMatch[1]) : (payload.days?.length || 0);
  ...
  const handleEdit = () => {
    alert('Chỉnh sửa lịch trình');
  };

  const handleShare = () => {
    alert('Chia sẻ lịch trình');
  };
  ...
  const handleDuplicate = () => {
    alert('Đã sao chép lịch trình');
  };

  const handleModifyInChat = () => {
    alert('Mở trong chat để chỉnh sửa');
  };
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3 text-muted-foreground">
                ...
                <span>•</span>
                <span>{totalDays} ngày {totalNights > 0 ? `${totalNights} đêm` : ''}</span>
                <span>•</span>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-6 mb-6">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-muted-foreground mb-1">Điểm đến</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-muted-foreground mb-1">Ngân sách dự kiến</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-muted-foreground mb-1">Khách sạn</p>
                      <p>{payload.hotel.name}</p>
                      {payload.hotel.price && (
                        <p className="text-sm text-muted-foreground">{payload.hotel.price}</p>
                      )}
                    </div>
                  </div>
                )}
              ...
              {/* Categorized Places */}
              {(categorizedPlaces.places?.length > 0 || categorizedPlaces.food?.length > 0 || categorizedPlaces.drink?.length > 0 || categorizedPlaces.coffee?.length > 0) && (
                <div className="mt-6 pt-6 border-t border-border">
                  <h4 className="mb-4">Địa điểm trong lịch trình</h4>
                  
                  {/* Places 🏛 */}
                  {categorizedPlaces.places && categorizedPlaces.places.length > 0 && (
                    <div className="mb-6">
                  ...
                  {/* Food 🍽 */}
                  {categorizedPlaces.food && categorizedPlaces.food.length > 0 && (
                    <div className="mb-6">
                      <h5 className="mb-3 flex items-center gap-2">
                        <span className="text-xl">🍽</span>
                        <span>Quán ăn</span>
                  ...
                  {/* Drink 🥤 */}
                  {((categorizedPlaces.drink && categorizedPlaces.drink.length > 0) || (categorizedPlaces.coffee && categorizedPlaces.coffee.length > 0)) && (
                    <div className="mb-6">
                      <h5 className="mb-3 flex items-center gap-2">
                        <span className="text-xl">🥤</span>
                ...
                <div className="ml-7 relative">
                  ...
                  <div className="space-y-6">
                    {day.activities.map((activity, index) => {
                      const category = activity.icon || activity.category || 'culture';
                      ...
                      if (index > 0) {
                        ...
                        if (previousEndTime && activity.travelTime) {
                          const travelMinutes = extractMinutes(activity.travelTime);
                          const timeMatch = previousEndTime.match(/(\d{1,2}):(\d{2})/);
                          if (timeMatch) {
                      ...
                      if (timeRange) {
                        durationToDisplay = timeRange.calculatedDurationMinutes;
                      ...
                      const formattedDuration = durationToDisplay !== null && durationToDisplay > 0
                        ? formatDuration(`${durationToDisplay} phút`) 
                        : null;
                      
                      // Ensure we always show both if we have at least one
                      const shouldShowTimeInfo = timeRange || formattedDuration;
                      
                      return (
                        <div key={activity.id || `act-${day.day}-${index}`} className="relative pl-8">
                        {/* Timeline Dot */}
                        <div className="absolute left-[-4px] top-6 w-2.5 h-2.5 rounded-full bg-[#0066FF] ring-4 ring-white" />
                          ...
                          {(() => {
                            ...
                            const previousActivity = index > 0 ? day.activities[index - 1] : null;
                            const travelTimeToDisplay = previousActivity?.travelTimeToNext || (index === 0 ? activity.travelTime : null);
                            
                            if (!travelTimeToDisplay) return null;
                            
                            // Determine transport mode icon based on travel time string or default to driving
                            const travelTimeLower = travelTimeToDisplay.toLowerCase();
                            let transportIcon = '🚗'; // Default to car
                            let transportText = 'xe';
                            
                            if (travelTimeLower.includes('xe máy') || travelTimeLower.includes('máy') || travelTimeLower.includes('motorbike')) {
                              transportIcon = '🏍️';
                              transportText = 'xe máy';
                            } else if (travelTimeLower.includes('bộ') || travelTimeLower.includes('đi bộ') || travelTimeLower.includes('walking')) {
                              transportIcon = '🚶';
                              transportText = 'đi bộ';
                            } else if (travelTimeLower.includes('taxi') || travelTimeLower.includes('xe') || travelTimeLower.includes('car') || travelTimeLower.includes('driving')) {
                              transportIcon = '🚗';
                              transportText = 'xe';
                            }
                            ...
                            return (
                              <div className="mb-3 flex items-center justify-center">
                                <div className="flex items-center gap-2 text-sm bg-[#0066FF]/5 text-[#0066FF] px-3 py-1.5 rounded-lg border border-[#0066FF]/10">
                                  ...
                                  <span>•</span>
                                  <span>{transportIcon} {transportText}</span>
                                </div>
                              </div>
                            );
                          })()}
                        ...
                        <div className="bg-gray-50 rounded-xl p-5 hover:shadow-md transition-shadow border border-gray-100">
                          <div className="flex items-start gap-4 mb-4">
                            {/* Category Icon */}
                              <div className={`w-12 h-12 rounded-xl ${getCategoryColor(category)} flex items-center justify-center shrink-0`}>
                              {(() => {
                            ...
                            <div className="flex-1 min-w-0">
                              <h4 className="mb-1 font-semibold">{activity.name}</h4>
                                ...
                                <div className="flex items-center justify-between mb-3">
                                  ...
                                  {(formattedDuration || timeRange) && (
                                    <div className="flex items-center gap-2 text-sm">
                                      ...
                                      <span className="text-muted-foreground">
                                        Dự kiến {formattedDuration || (timeRange ? formatDuration(`${timeRange.calculatedDurationMinutes} phút`) : '')}
                                      </span>
                                    </div>
                                  )}
                                </div>
                              ...
                              {activity.rating && (
                                <div className="flex items-center gap-2 text-sm mb-3">
                                  ...
                                  <span className="text-muted-foreground">{activity.rating}/5</span>
                                </div>
                              )}
                            </div>
                          </div>
                            ...
                            {activity.cost && (
                              <div className="flex items-center gap-2 text-sm mb-4">
                                ...
                                <span className="text-green-700">
                                  {activity.cost === 'Miễn phí' || activity.cost === '0' ? 'Miễn phí' : activity.cost}
                                </span>
                              </div>
                            )}
                          ...
                          {activity.notes && (
                            <div className="bg-white rounded-lg p-3 border border-border">
                              <p className="text-sm text-muted-foreground">
                                💡 <span className="font-medium">Gợi ý:</span> {activity.notes}
                              </p>
                            </div>
                          )}
                        </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-24">
              ...
              <div className="aspect-square bg-gradient-to-br from-[#0066FF]/10 to-[#00C29A]/10 rounded-xl flex items-center justify-center mb-4">
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground">Xem bản đồ tương tác</p>
                </div>
              </div>
              ...
              <div className="mt-6 pt-6 border-t border-border space-y-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {payload.days?.reduce((acc, day) => acc + (day.activities?.length || 0), 0) || 0} địa điểm
                  </span>
                </div>
                {payload.days && (
                <div className="flex items-center justify-between text-sm">
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between pb-3 border-b border-border">
                      <span className="text-sm text-muted-foreground">Ngân sách tối thiểu</span>
                      <span className="font-medium">{(payload.budget_min / 1000000).toFixed(1)} triệu VNĐ</span>
                </div>
                <div className="flex items-center justify-between pb-3 border-b border-border">
                      <span className="text-sm text-muted-foreground">Ngân sách tối đa</span>
                      <span className="font-medium">{(payload.budget_max / 1000000).toFixed(1)} triệu VNĐ</span>
                  </div>
                  </>
                ) : (
                  <div className="pb-3 border-b border-border">
                    <span className="text-sm text-muted-foreground">{payload.budget || 'Chưa xác định'}</span>
                  </div>
                )}
              </div>
            </div>
